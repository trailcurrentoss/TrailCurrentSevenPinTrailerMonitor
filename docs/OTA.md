# OTA Overview
The OTA update process will be as follows:
1. The device name needs to be captured for a specific "module" or "modules" if they are to receive the same firmware.
2. The cloud service will notify the client of a new version via MQTT
3. The client (Raspberry Pi) will download a .zip file from the OTA cloud containing all modules firmwares in folders
4. A script will be executed that will send a CAN Bus message to a "node" letting it know that it's time for an update.
5. The can bus message will cause the ESP module to reboot. By default firmware will be set to wait 30 seconds on boot for OTA.
6. After a delay giving the module time to reboot but not to exceed the 30 second window (can be adjusted if needed.) 
7. The script will run for that modules firmware and move on
8. Repeat until all modules are updated

**TODO**: Still need to see if a more robust error handling can occur. Also need to check on how to see if an udate was applied successfully. Possibly need a "deviceId" => version mapping of CAN 


The following is the python script that is used to send the updates. It will be called per MCU module after the restart.
``` python
